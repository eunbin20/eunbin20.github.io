...
> _막연하게 타입에 ?으로 표시하면 undefined를 유니온타입을 추가한 것과 유사하지않을까?_ 라고 생각하고있었다.
...
위와 같이 즉시실행함수의 형태로 객체를 만들게 되는데, 이 코드는 번들러가 '사용하지 않는'코드라고 판단하지 않으므로 트리쉐이킹되지 않고 최종 번들에 포함된다. 즉, 개발 환경에서만 필요한 불필요한 타입 정의 코드로 인해 번들의 크기가 증가하게 되는 것이다.
...
Typescript의 타입 시스템은 완벽하지 않다. Typescript에서는 기본적으로 모든 배열의 멤버에 대한 접근에 대해 해당 배열의 멤버를 반환한다고 가정한다. 즉, 어떤 배열의 해당 인덱스에 실제로 값이 존재하지 않아도 존재한다고 가정한다. (자바스크립트에서도 배열에 대해서는 비슷한 방식으로 동작한다. 만약, 배열의 길이보다 큰 인덱스로 배열 요소에 접근하면 레퍼런스 에러가 아닌 undefined를 제공한다.)

```jsx
function withElement(elements: string[]) {
  console.log(elements[9001].length); // 타입 오류 발생하지 않음
}
...
지금까지 개발을 하는 과정에서는 설령 위와 같은 상황이 생겼더라도 타입 오류가 발생하지 않는 것에 대해 큰 의문을 갖지 않았을 것 같다. 하지만 이 책을 통해 예제를 접해보니 타입스크립트를 기술적으로 불안정하다고 하는 것의 의미를 이해할 수 있었고 흥미로웠다.

이번 파트와 예제를 통해 타입스크립트와 자바스크립트가 완벽하지 않다는 걸 더 잘 이해하게 되었고, 이 사실을 알고 나니 앞으로 이런 코드 작성할 때 조금 더 신경 쓰게 될 것 같다.
