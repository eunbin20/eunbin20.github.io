...
- DFP(**DoubleClick for Publishers**)
  - Google Ad Manager로 리브랜딩됨. 둘 다 같은 의미임. 두 단어 모두 쓰이고 있음.
  - 인벤토리를 정의하고 광고 캠페인을 생성, 관리, 보고하는 솔루션
- Ad Exchange: 구글 광고 네트워크
- Google Ad Manager(DFP와 Ad Exchange가 통합되어 리브랜딩 됨)
  - 광고 게시자가 광고 인벤토리를 판매, 예약, 전달, 및 관리할 수 있는 광고 플랫폼
  - GA, Adsense와 연동되어 유용하고 강력한 기능을 제공
  - 접 광고 물량을 진행하는 매체(publisher)가 애드센스와 애드 익스체인지(ad exchange) 등의 다양한 광고 네트워크를 데스크톱, 모바일웹, 앱에서 관리할 수 있도록 돕는 통합 광고 플랫폼(서버)
  - 무료 광고 서버를 이용해 직접광고가 부족한 잔여 인벤토리를 구글 광고 네트워크로 채울 수 있음
  - 구글 광고 네트워크와 제3자 광고 네트워크를 경쟁시켜 애드센스 하나만 운영하는 것보다 더 높은 광고 CPM을 얻을 수 있음
- Ad Unit(광고 단위)
  - 광고 소재를 게재할 수 있는 공간/영역
  - 광고 단위를 기반으로 GPT 태그를 만들어 소스코드에 추가한다.
- Line item(광고 항목)
  - 광고가 나가는 시기와 방법 및 타게팅(광고 인벤토리, 국가, 플랫폼 등)을 결정
- Creatives(광고 소재)
  - 실제 광고에 노출될 광고 소재
  - 이미지, 텍스트, 동영상 등 유저가 실제로 보게 될 광고 소재

# 구글 애드매니저 설정

DFP는 인벤토리를 정의하고 광고 캠페인을 생성, 관리, 보고하는 솔루션입니다. 인벤토리는 광고를 노출할 수 있는 영역을 뜻합니다.

인벤토리의 기본 요소**광고 단위 : AD Unit**

생성된 인벤토리(Inventory)는 광고 단위(AD Unit)라고 부릅니다. 광고 단위는 광고 소재를 게재할 수 있는 공간을 뜻합니다. 아래 이미지에서 노란색으로 표시한 영역이 바로 광고 단위입니다.
...
**그러나, 위 단계대로 설정하였으나 GPT 태그를 웹사이트에 삽입하여도 페이지에 리워드 광고가 게재되지 않음**
...
라는 답변을 받았다. 해당 절차 완료 후에 광고를 정상적으로 게재되는 것을 확인하였다 🎉
